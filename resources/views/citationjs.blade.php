...
<div id="wrapper">
    ...
    <div id="header">
        <h1 id="title"><a href="{{url('alljs')}}" title="CiteSeerX"><img
            ...
            <div id="sidebar">
                ...
                <div id="sorting">
                    ...
                    <form method="post" action="./upload" enctype="multipart/form-data" 
                      enctype="application/x-www-form-urlencoded" style="margin-top: 20px" >
                        <input type="file" name="citation" accept="application/pdf"/>
                        <input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
                        <button style="margin-top: 20px" type="submit" class="btn btn-sm btn-info">Upload</button>
                    </form>
                </div>

            </div>
            <div id="result_list">
                <h3>Các bài viết trích dẫn của <em style="color: #0078A4">{{$js->title}}</em></h3>
                @for($i = 0; $i < sizeof($citetojs); $i++)
                    <div class="result">
                        <h3>

                            <a class="remove doc_details"
                               href="#">
                                <em>{{$citetojs[$i]->titleOnGoogle}}</em>
                            </a>
                        </h3>
                        <div class="snippet" style="margin-top: 10px">
                            @if($citetojs[$i]->cluster_id != "")
                                <b>ClusterID trên google scholar: </b>
                                <p>{{$citetojs[$i]->cluster_id}}</p>
                            @endif
                            <b>MLA: </b>
                            <p>{{$citetojs[$i]->mla}}</p>
                            ...
                            <b>Chicago: </b>
                            <p>{{$citetojs[$i]->chicago}}</p>
                            
                            <b>Harvard: </b>
                            <p>{{$citetojs[$i]->harvard}}</p>

                            <b>Vancouver: </b>
                            <p>{{$citetojs[$i]->vancouver}}</p>
                        </div>
                        <br>
                        <div class="pubextras">
                            <p>Citation {{$citetojs[$i]->cites}}</p>
                        </div>
                        <div class="pubtools">
                            <span class="Z3988"
